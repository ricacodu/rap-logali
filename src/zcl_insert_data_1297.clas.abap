CLASS zcl_insert_data_1297 DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
  ...
  PRIVATE SECTION.
    ...
    TYPES items TYPE TABLE OF zitems_so_1297 WITH KEY id.
    ...
    METHODS get_random_date
      IMPORTING
                num           TYPE i
      RETURNING VALUE(result) TYPE zde_createon_1297.

    METHODS get_random_country
      IMPORTING
                num           TYPE i
      RETURNING VALUE(result) TYPE zde_country_1297.

    METHODS get_random_status
      IMPORTING
                num           TYPE i
      RETURNING VALUE(result) TYPE zde_orderstatus_1297.

    METHODS randomize_header
      IMPORTING
                num           TYPE i
      RETURNING VALUE(result) TYPE zheaders_so_1297.

    METHODS randomize_items
      IMPORTING
        id_header TYPE zheaders_so_1297-idheader
        num       type i
      RETURNING
...
CLASS zcl_insert_data_1297 IMPLEMENTATION.


  METHOD if_oo_adt_classrun~main.
    DATA: lt_so_header TYPE TABLE OF zheaders_so_1297.
  ...
  METHOD randomize_header.
    DATA: lv_firstname TYPE zheaders_so_1297-firstname,
          lv_lastname  TYPE zheaders_so_1297-lastname,
          lv_email     TYPE zheaders_so_1297-email,
          lv_IMAGEURL  TYPE zheaders_so_1297-imageurl.
...
*    lv_IMAGEURL = 'https://raw.githubusercontent.com/SAP-samples/fiori-elements-opensap/main/week1/images/airlines/Green-Albatross-logo.png'.
    ...
    result  = VALUE zheaders_so_1297( idheader = get_id( )
                                      email = lv_email
  ...
  METHOD delete_entries.
    ...
    DELETE FROM zheaders_so_1297.
    IF sy-subrc = 0.
      gv_console->write( 'ZHEADERS_SO_1297 cleared.' ).
    ELSE.
      gv_console->write( 'ZHEADERS_SO_1297 not cleared.' ).
    ENDIF.

    DELETE FROM zITEMS_so_1297.
    IF sy-subrc = 0.
      gv_console->write( 'ZITEMS_SO_1297 cleared.' ).
    ELSE.
      gv_console->write( 'ZITEMS_SO_1297 not cleared.' ).
    ENDIF.
  ENDMETHOD.

  METHOD insert_entries.
    DATA ls_header TYPE zheaders_so_1297.
    DATA lt_items TYPE TABLE OF zITEMS_so_1297.
    ...
    DO 10 TIMES.
      ...
      lt_items = randomize_items( id_header = ls_header-idheader
                                  num = sy-index ).

      INSERT zheaders_so_1297 FROM @ls_header.
      IF sy-subrc = 0.
        gv_console->write( 'ZHEADERS_SO_1297 insert ok.' ).
      ELSE.
        gv_console->write( 'ZHEADERS_SO_1297 insert not ok.' ).
      ENDIF.

      INSERT zITEMS_so_1297 FROM TABLE @lt_items.
      IF sy-subrc = 0.
        gv_console->write( 'ZITEMS_SO_1297 insert ok.' ).
      ELSE.
        gv_console->write( 'ZITEMS_SO_1297 insert not ok.' ).
      ENDIF.
...
METHOD randomize_items.
    FIELD-SYMBOLS <item> TYPE zITEMS_so_1297.

    DATA lv_idpos TYPE zITEMS_so_1297-id.
    data: lv_index(2).
